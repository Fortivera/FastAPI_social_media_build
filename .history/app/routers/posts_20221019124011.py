...
@router.get('/{id}', response_model=List[schemas.PostBack])
def get_posts(id: int, db: Session = Depends(get_db), current_user: str = Depends(oauth2.get_current_user), like: int = Depends(models.Like)):
