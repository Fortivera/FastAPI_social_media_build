...
@app.put('/posts', status_code=status.HTTP_201_CREATED)
def create_posts(post: Post, id: int):
    cursor.execute(
        """UPDATE posts SET (title, content, published) VALUES (%s,%s,%s) RETURNING * """, (post.title, post.content, post.published))
    updated_post = cursor.fetchone()
    conn.commit()

    if updated_post == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"post with {id}id was not found", (str(id)))
