...
# collection of test performed on
...
def test_posting_as_unauthorized(client, test_posts, test_user):
    response = client.get(
        '/posts/', json={'title': "1title", 'content': "1content", 'published': "1published"})  # using the schemas format for post
    assert response.status_code == 401
...
def test_post_update_other_user(authorized_client, test_user, test_posts, test_user2):
    data = {
        'title': 'updated title',
        'content': 'updated content',
        'id': test_posts[3].id
...
def test_unauthorized_post_update(client, test_user, test_posts, test_user2):
    data = {
        'title': 'updated title',
        'content': 'updated content',
        'id': test_posts[0].id
...
def test_authorized_nonexistent_post(authorized_client, test_user, test_posts, test_user2):
    data = {
        'title': 'updated title',
        'content': 'updated content',
        'id': test_posts[0].id
    }
    response = authorized_client.put(f'/posts/{0000}', json=data)
    assert response.status_code == 404
